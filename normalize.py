...
class problem_validator(base_validator):
    ...
    def _normalize_default_setter_set_last_verified_live(self, document) -> str | None:
        ...
        url = url + document["git_repo"] + ("/commit/", "commit/")[document["git_repo"].endswith("/")]
        url = url + document["git_hash"]
        try:
            response: Response = head(url, allow_redirects=True, timeout=5)
            return datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%S.%fZ') if response.status_code == 200 else document.get("last_verified_live")
        except ConnectionError:
            return document.get("last_verified_live")


with open(join(dirname(__file__), "tests/data/problem_format.json"), "r", encoding="utf8") as file_ptr:
...
for problem in EGP_PROBLEMS:
    if not validator.validate(problem):
        ...
        sys_exit(1)
with open(join(dirname(__file__), "egp_problems.json"), "w", encoding="utf8") as file_ptr:
    dump([validator.normalized(problem) for problem in EGP_PROBLEMS], file_ptr, indent=4, sort_keys=True)
